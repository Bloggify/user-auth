...
export default async (conf) => {
    ...
    if (!User) {
        throw new Error("Please define an Sequelize model called 'User'")
    }

    passportConfig.verify_callback = async (email, password, cb) => {
        try {
            ...
            if (!user.confirmed) {
                throw new Error("The registration of this user is not confirmed.")
            }
    ...
    await Bloggify.pluginLoader.load([
        "bloggify-passport", passportConfig
    ])
